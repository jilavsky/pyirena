...
def test_schema_names_unique_and_indexed():
    ...
    assert len(names) == 84, f"expected 84 control schemas, found {len(names)}"
...
def test_mcp_registers_all_tools_structurally():
    ...
    server registers 101 (18 read + 83 control). Here we lock the counts and
    ...
    assert len(names) == 101, f"expected 101 registered MCP tools, found {len(names)}"
    assert len(ctrl_tools) == 83, f"expected 83 control tools, found {len(ctrl_tools)}"

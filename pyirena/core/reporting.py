"""
pyirena.core.reporting — the one Markdown fit-report builder.

Every numeric result pyIrena produces should be reachable as text, not only as
an HDF5 group: Igor Irena wrote each fit to the notebook, and users expect a
trail they can paste into an e-mail, a logbook or a paper draft.  This module
builds that text.

It lives in ``core`` because three layers need it and none of them may import
each other's: the **GUI** (Data Selector "Create Report", and the Copy/Save
buttons on the fit panels), the **api/control** layer (``export_fit_report``,
used by AI agents over MCP), and any batch script.  It is therefore Qt-free and
matplotlib-free — numpy and the standard library only.

Input is the dict shape returned by ``pyirena.io.load_<tool>_results()``, which
is also what ``pyirena.api.results`` returns.  Every section is optional: pass
only ``fit_results`` and you get a Unified Fit report, pass everything and you
get the full multi-tool report the Data Selector writes.

Moved here from ``pyirena/gui/data_selector/report.py`` (which now re-exports
it) with no change to the generated text.
"""
import logging
...
def _build_report(file_path: str,
    ...
    if waxs_peakfit_results is not None:
        ...
        L += [
            ...
            f"| DOF | {wp['dof'] if wp.get('dof') is not None else 'N/A'} |",
        ...
        for i, pk in enumerate(peaks_list):
            ...
            if area_v is None:
                ...
                # always carry the derived value.  A shape this build does not
                # know (a file from a newer/older pyIrena) must cost one table
                # cell, not the whole report.
                ...
                try:
                    ...
                    log.debug("peak area unavailable for shape %r", shape,
                              exc_info=True)
                    area_v = area_s = None
            # "± 0" reads as a measured zero; an uncomputed uncertainty is "—"
            # (same rule as the Unified level tables).
            area_std_str = f"± {_wp_fmt(area_s, '.3g')}" if area_s else "—"
...
def build_report(file_path: str, **sections) -> str:
    ...
    Every section is optional, so a single panel can report just its own tool::

        build_report(path, data_info=..., fit_results=unified_dict)

    Args:
        file_path: Source file the results came from; only its basename is
    ...
    Returns:
        Markdown text, ready for the clipboard or a ``.md`` file.
    """
    return _build_report(file_path, **sections)

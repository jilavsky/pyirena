...
def _build_report(file_path: str,
                  ...
                  waxs_peakfit_results: Optional[dict] = None) -> str:
    """
...
class DataSelectorPanel(QWidget):
    ...
    def create_report(self):
        ...
        if not show_data and not show_fit and not show_sizes and not show_simple_fits and not show_waxs_peakfit:
            self.status_label.setText(
                "Nothing to report — check 'Data', 'Unified Fit', 'Size Dist.', 'Simple Fits' or 'WAXS Peaks' checkbox"
            )
            return
    ...
    def tabulate_results(self):
        ...
        if not show_unified and not show_sizes and not show_simple_fits and not show_waxs_peakfit:
            self.status_label.setText(
                "Nothing to tabulate — check 'Unified Fit', 'Size Dist.', 'Simple Fits' or 'WAXS Peaks' checkbox"
            )
            return
        ...
        loaded = []   # list of (fname, uf, sz, sf, wp)
        max_levels   = 0
        max_wp_peaks = 0
        ...
        for fp in file_paths:
            ...
            loaded.append((fname, uf, sz, sf, wp))
        ...
        rows = []
        for fname, uf, sz, sf, wp in loaded:
            row = [fname]

...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "\n\n"
        "DATA OPERATIONS (dispatcher category 'data'): the only pyirena "
        "tools that CREATE data files. pyirena_call('average_data', {...}) "
        "averages frames (set similarity_check=true to drop "
        "radiation-damaged ones), 'subtract_data' removes a buffer or "
        "baseline, 'merge_datasets' joins USAXS and SAXS into one curve, and "
        "'scale_data'/'trim_data'/'rebin_data'/'divide_data' do the rest. "
        "Output goes to a sibling of the source folder ('..._manip' or "
        "'..._merged') with a per-operation filename suffix (_avg, _sub, "
        "_div, _scaled, _trimmed, _rebinned, _merged); pass output_folder to "
        "override. Repeating an operation overwrites its own previous "
        "result. Check the returned n_dropped_nonpositive — a subtraction "
        "that strips many points is over-subtracted. If a data operation is "
        "not in this category, pyirena cannot do it: say so rather than "
        "searching the other categories. "
        "\n\n"
        ...
        "'waxs' (the 'calculators' and 'data' categories are described "
        "above). Prefer "
